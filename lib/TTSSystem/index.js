...
export class TTSEngineManager {
  ...
  /**
   * Synthesize text using the primary or secondary engine.
   * @param {string} text
   * @param {{ secondary?: boolean; voice?: string; pitch?: number; rate?: number; [key: string]: any }} [options]
   */
  ...
  setVolume(volume, isSecondary = false) {
    const engine = /** @type {any} */ (isSecondary ? this.secondaryEngine : this.primaryEngine)
    if (engine && typeof engine.setVolume === 'function') engine.setVolume(volume)
  }
}
