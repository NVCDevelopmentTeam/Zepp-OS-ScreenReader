...
export class ScreenReaderUI {
  ...
  createFocusRect() {
    ...
    const { width, height } = getDeviceInfo()
    // "Cursor Style" (settingsKey 'cursorStyle', setting/NavigationSetting.js)
    // was previously ignored - the focus indicator always drew as a
    // green stroked border regardless of this setting.
    const style = globalThis.ScreenReaderConfig?.cursorStyle || 'border'
    if (style === 'none') {
      this.focusRect = null
      return
    }
    const isHighlight = style === 'highlight'
    // "Cursor Color" (settingsKey 'cursorColor', hex string without '#')
    // lets a low-vision sighted helper pick a color that shows up well
    // against whatever's on screen, or a color that's less visually
    // jarring than the fixed green used before.
    const colorHex = globalThis.ScreenReaderConfig?.cursorColor || '00ff00'
    const color = parseInt(colorHex, 16) || 0x00ff00
    try {
      this.focusRect = this.rootGroup.createWidget(widget.RECT, {
        ...
        h: height,
        color,
        line_width: isHighlight ? 0 : 4,
        alpha: isHighlight ? 90 : 255,
        style: isHighlight ? widget.prop.STYLE_FILL : widget.prop.STYLE_STROKE,
        z_index: 9998,
        visible: false
      })
    } catch (error) {
  ...
  updateFocusRect(bounds) {
    // "none" style intentionally never creates a focusRect (see
    // createFocusRect above), so there's nothing to update or show.
  ...
  setupGestures() {
    ...
    this.interactionLayer.addEventListener(event.CLICK_UP, (info) => {
      ...
      // A hold of >=500ms with negligible movement previously matched
      // neither isTap (too long) nor isSwipe (didn't move) and was
      // silently dropped - so LONG_PRESS (mapped to "open context menu"
      // in lib/interaction/gesture.js) could never actually fire.
      const isLongPress = duration >= 500 && Math.abs(deltaX) < 30 && Math.abs(deltaY) < 30

      if (isLongPress && touchCount === 1) {
        log.info('1-finger long press')
        globalThis.GestureHandlerInstance?.handleGesture(GESTURES.LONG_PRESS)
      } else if (isTap) {
        if (touchCount === 1) {
