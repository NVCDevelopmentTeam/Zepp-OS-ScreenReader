...
class BrailleService {
  ...
  async output(text) {
    // "Braille Output" master toggle (settingsKey 'brailleEnabled',
    // AudioBrailleSetting.js) - previously checked a different,
    // never-set key ('vibrationBraille') and didn't gate the Bluetooth
    // display path at all, so this master switch had no effect either
    // way.
    if (globalThis.ScreenReaderConfig?.brailleEnabled === false) return
    ...
    // No physical display connected - fall back to vibration-pattern
    // braille as a best-effort substitute.
