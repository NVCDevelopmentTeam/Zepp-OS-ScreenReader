...
class NavigationManager {
  ...
  async navigate(direction) {
    try {
      ...
      if (mode === 'speech_rate') {
        ...
        if (screenReader) {
          await screenReader.speak(`Rate ${Math.round(currentRate * 100)}%`, {
            priority: 'high',
            force: true
          })
        }
    ...
    try {
      if (options.is_password) {
        ...
        if (config.readPasswords === true) {
          text = rawText || 'Blank password'
        } else {
          const len = rawText ? rawText.length : 0
          return len > 0 ? `Password field, ${len} bullets` : 'Password field, blank'
        }
      }
      ...
      switch (type) {
        ...
        case widget.PICKER:
          ...
          break
        case widget.SLIDER:
          text = options.text || options.accessibility_text || 'Slider'
          break
  ...
  async readCenterElement() {
    ...
    if (closestIndex !== -1) {
      ...
      const screenReader = this.getScreenReader()
      if (screenReader) {
        await screenReader.speak('No element found at center of screen', { priority: 'high' })
      }
    }
  }
